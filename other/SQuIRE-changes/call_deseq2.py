...
#Count graphs
trans_index <- res$padj < 0.05
trans_index[is.na(trans_index)] <- FALSE
trans <- rownames(res)[trans_index]

pdf("count_graph_all.pdf")
for(name in trans){
plotCounts(dds, gene=name, intgroup="condition")
}
dev.off()

trans_te<-trans[grep(":", trans)]
if(length(trans_te)>0){
pdf("count_graph_TE_only.pdf")
for(name in trans_te){
plotCounts(dds, gene=name, intgroup="condition")
}
dev.off()
}
